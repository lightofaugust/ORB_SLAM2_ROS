...
class ImageGrabber {
  public:
    ImageGrabber(ORB_SLAM2::System *pSLAM) : mpSLAM(pSLAM) {mpRosTfInterface = new ORB_SLAM2::TF2Interface();}

    void GrabRGBD(const sensor_msgs::ImageConstPtr &msgRGB,
                  const sensor_msgs::ImageConstPtr &msgD);

    ORB_SLAM2::System *mpSLAM;
        ORB_SLAM2::TF2Interface *mpRosTfInterface;
};
...
int main(int argc, char **argv) {
    ...
    if (argc != 3) {
        cerr << endl
             << "Usage: rosrun ORB_SLAM2 RGBD path_to_vocabulary "
                "path_to_settings"
             << endl;
        ros::shutdown();
        return 1;
    ...
    ImageGrabber igb(&SLAM);
        //igb.mpRosTfInterface->LookupCameralinkToRgbOptical();
...
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr &msgRGB,
    ...
    try {
        ...
        cv::Mat Tcw = mpSLAM->TrackRGBD(cv_ptrRGB->image, cv_ptrD->image,
                          cv_ptrRGB->header.stamp.toSec());
        mpRosTfInterface->PublishCurrentFrameTcw(Tcw);
}
